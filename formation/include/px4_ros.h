...
class command_to_mavros
{
        ...
        command_nh("~")
    {
        ...
        //  修改  本话题来自外部小觅摄像头  
        position_sub = command_nh.subscribe("/vins_estimator/odometry", 100, &command_to_mavros::pos_cb,this);

        // 【订阅】无人机当前速度 坐标系:ENU系
        //  本话题来自飞控(通过Mavros功能包 /plugins/local_position.cpp读取), 对应Mavlink消息为LOCAL_POSITION_NED (#32), 对应的飞控中的uORB消息为vehicle_local_position.msg
        //  修改  本话题来自外部小觅摄像头  
        velocity_sub = command_nh.subscribe("/vins_estimator/odometry", 100, &command_to_mavros::vel_cb,this);
    ...
    private:
        ...
        void pos_cb(const nav_msgs::OdometryPtr& msg)
        {
            pos_drone_fcu  = Eigen::Vector3d(msg->pose.pose.position.x,msg->pose.pose.position.y,msg->pose.pose.position.z);
        }

        void vel_cb(const nav_msgs::OdometryPtr& msg)
        {
            vel_drone_fcu = Eigen::Vector3d(msg->twist.twist.linear.x,msg->twist.twist.linear.y,msg->twist.twist.linear.z);
        }
