...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber uav2_state_sub = nh.subscribe<mavros_msgs::State>("/uav1/mavros/state", 10, uav1_state_sub_cb);
    ros::Subscriber uav2_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav1/mavros/local_position/pose", 100, uav1_position_sub_cb);
    ros::Subscriber uav2_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav1/mavros/local_position/velocity_local", 100, uav1_velocity_sub_cb);
    ros::Publisher uav2_setpoint_raw_local_pub = nh.advertise<mavros_msgs::PositionTarget>("/uav1/mavros/setpoint_raw/local", 10);
...
void prinft_drone_state(float current_time)
{
    ...
    cout << "att_uav0      [R P Y] : " << att_drone_uav0[0] / M_PIl * 360 << " [du ] "<< att_drone_uav0[1] / M_PIl * 360<<" [du ] "<< att_drone_uav0[2] / M_PIl * 180<<" [du ] "<<endl;
